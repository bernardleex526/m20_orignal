...
bool LinePlanner::isLineMode(
  ...
  for (std::size_t i = 1; i < global_path.size() && dist_accum < lookahead_distance; ++i) {
    ...
    if (dtheta > params_.lineplanner_yaw_threshold) {
      return false;
    }

    dist_accum += std::sqrt(
        (global_path[i].x() - global_path[i-1].x()) * (global_path[i].x() - global_path[i-1].x()) +
        (global_path[i].y() - global_path[i-1].y()) * (global_path[i].y() - global_path[i-1].y()));
    prev_theta = theta;
  }
...
VelocityCommand LinePlanner::computeLineCommand(
  ...
  Scalar vx = std::min(target_speed, dist_to_end * 0.5);
  cmd.vx = math::clamp(vx, -params_.max_linear_vel_x, params_.max_linear_vel_x);

  // Lateral velocity: P-controller for cross-track error
  // v_y = −K_p · e_y  (negative to move toward the line)
  cmd.vy = math::clamp(-params_.lineplanner_lateral_gain * cross_track,
                        -params_.max_linear_vel_y, params_.max_linear_vel_y);
  ...
  cmd.omega = math::clamp(heading_error * 0.5,
                           -params_.max_angular_vel, params_.max_angular_vel);
